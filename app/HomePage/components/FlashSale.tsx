...
import { ArrowRight, ShoppingBag, Check, ShoppingCart, Timer, Zap } from "lucide-react";
...
export default function FlashSale() {
  ...
  const [toast, setToast] = useState<{ visible: boolean; productName: string }>({ visible: false, productName: "" });
          ...
          const mapped = items.map((item: any) => {
            ...
            if (activePromo) {
                ...
                if (activePromo.discount_type === 'percent') {
                     ...
                     discPercent = Math.round(((origPrice - finalPrice) / origPrice) * 100);
                }
            }
        ...
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-8 sm:mb-10 md:mb-14 gap-6">
          <div>
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <span
                className="text-red-400 font-bold text-[11px] uppercase tracking-[0.25em]"
              >
                Penawaran Terbatas
              </span>
            </div>
            ...
            >
              Flash <em className="font-semibold not-italic" style={{ fontStyle: "italic" }}>Sale</em>
            </h2>
            ...
            {timeLeft && (
               <div className="flex items-center gap-3 mt-4">
                  <div className="flex items-center gap-1.5 flex-col">
                      ...
                      <span className="text-[10px] text-white/50 uppercase tracking-widest">Jam</span>
                  </div>
                  <span className="text-white text-xl font-bold self-start mt-2">:</span>
                  <div className="flex items-center gap-1.5 flex-col">
                      <div className="bg-white/10 backdrop-blur-md border border-white/20 text-white w-12 h-12 flex items-center justify-center text-xl font-bold rounded-lg shadow-inner">
                          {String(timeLeft.minutes).padStart(2, '0')}
                      </div>
                      <span className="text-[10px] text-white/50 uppercase tracking-widest">Mnt</span>
                  </div>
                  <span className="text-white text-xl font-bold self-start mt-2">:</span>
                  <div className="flex items-center gap-1.5 flex-col">
                      ...
                      <span className="text-[10px] text-white/50 uppercase tracking-widest">Dtk</span>
                  </div>
               </div>
            )}
          </div>
