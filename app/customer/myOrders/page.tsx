...
export default function MyOrdersPage() {
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: rawOrders.filter((o) => o.status?.toUpperCase() === "PENDING")
        .length,
      completed: rawOrders.filter((o) =>
  ...
  const handleOrderDetail = async (orderId: string) => {
    ...
    try {
      const res = await api.get(
        `/customer/orders/${orderSummary.order_number}`,
      );
      if (res.data.status === "success") {
        ...
        const addressData = o.shipping_address
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-stone-500 font-medium">
            Kelola dan pantau status pesanan harian Anda.
          </p>
        </div>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 border-b border-stone-100">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Cari ID/No. Pesanan..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="pl-9 pr-4 py-2 bg-stone-50 border border-stone-200 rounded-xl text-xs text-gray-900 focus:ring-2 focus:ring-stone-200 focus:outline-none w-full sm:w-64"
            />
          </div>
        </div>
