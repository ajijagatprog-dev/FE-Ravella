...
export default function ProductContentPage() {
    ...
    try {
      ...
      if (res.data.status === "success") {
        ...
        const mapped = fetchedData.map((item: any) => {
          const primaryMedia =
            item.media?.find((m: any) => m.is_primary) || item.media?.[0];
          const thumbnail =
            primaryMedia?.url ||
            item.image ||
          ...
          return {
            ...
            inStock:
              ...
              (item.variants &&
                ...
                item.variants.some((v: any) => v.stock > 0)),
            stock: item.stock || 0,
            isNew: true,
  ...
  const openEdit = (p: any) => {
    ...
    const existingVariants: VariantItem[] = (p._variants || []).map(
      (v: any) => ({
        id: v.id,
  ...
  const handleRemoveVariantMedia = (
    variantIndex: number,
    mediaIndex: number,
  ) => {
    setVariants((prev) =>
      prev.map((v, i) => {
        ...
        const newDeleteIds = item.id
          ? [...v.deleteMediaIds, item.id]
          : v.deleteMediaIds;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append(
        "stock",
        (editData.stock === "" ? 0 : (editData.stock ?? 100)).toString(),
      );
      ...
      if (
        ...
        Object.keys(editData.specifications).length > 0
      ) {
        const cleanSpecs = Object.fromEntries(
          Object.entries(editData.specifications).filter(
            ([k, v]) => k.trim() !== "" && v.trim() !== "",
          ),
        );
      ...
      if (variants.length > 0) {
        ...
        variants.forEach((v, idx) => {
          ...
          if (v.deleteMediaIds.length > 0) {
            formData.append(
              `delete_variant_media_ids_${idx}`,
              JSON.stringify(v.deleteMediaIds),
            );
          }
        });
      }
  ...
  const addFeature = () =>
    setEditData((d) => ({ ...d, features: [...d.features, ""] }));
  ...
  const updateFeature = (i: number, v: string) =>
    setEditData((d) => ({
      ...d,
      features: d.features.map((f, idx) => (idx === i ? v : f)),
    }));
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200/80">
                <th className="text-left text-[11px] font-bold uppercase tracking-wider text-slate-500 px-6 py-4">
                  Produk
                </th>
                <th className="text-left text-[11px] font-bold uppercase tracking-wider text-slate-500 px-4 py-4">
                  Kategori
                </th>
                <th className="text-left text-[11px] font-bold uppercase tracking-wider text-slate-500 px-4 py-4">
                  Harga
                ...
                <th className="text-center text-[11px] font-bold uppercase tracking-wider text-slate-500 px-4 py-4">
                  Aksi
                </th>
              </tr>
            </thead>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-slate-900 text-sm truncate max-w-[260px]">
                            {p.name}
                          </p>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-2">
          <p className="text-sm font-medium text-slate-500 order-2 sm:order-1">
            Menampilkan{" "}
            <span className="text-slate-900 font-bold">{paginated.length}</span>{" "}
            dari{" "}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm pt-10 px-4 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl mb-10 border border-slate-200/60">
            ...
            <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">
                  <ImageIcon size={12} className="inline mr-1" /> Gambar & Video
                  Utama Produk
                </label>
                <p className="text-[10px] text-slate-400 mb-2">
                  Maks 8 file (gambar + video). Klik ⭐ untuk set gambar utama.
                  Video maks 50MB.
                </p>
                      ...
                      >
                        {item.type === "image" ? (
                          ...
                          <div className="w-full h-full bg-slate-900 relative">
                            ...
                            <span className="absolute bottom-1 left-1 text-[9px] text-white bg-black/60 px-1.5 py-0.5 rounded font-bold pointer-events-none">
                              VIDEO
                            </span>
                          </div>
                        )}
                        ...
                        {item.is_primary && (
                          <span className="absolute top-1 left-1 text-[8px] font-bold bg-blue-500 text-white px-1.5 py-0.5 rounded">
                            UTAMA
                          </span>
                        )}
                      </div>
                ...
                {mediaItems.length < 8 && (
                  <label className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 text-sm text-slate-500 cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
                    <Upload size={16} />
                    <span>
                      Klik untuk upload gambar / video ({mediaItems.length}/8)
                    </span>
                    <input
                      type="file"
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Format: JPG, PNG, WebP (max 10MB) · MP4, WebM (max 50MB)
                </p>
              </div>
              ...
              <div className="border-t border-slate-100 pt-5">
                ...
                <p className="text-[10px] text-slate-400 mb-3">
                  Tambahkan varian produk (misal: warna berbeda) dengan foto,
                  harga, dan stok terpisah.
                </p>
                ...
                {variants.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {variants.map((variant, vIdx) => (
                      <div
                        key={vIdx}
                        className={`rounded-2xl border-2 p-4 space-y-3 transition-all ${variant.is_default ? "border-blue-300 bg-blue-50/30" : "border-slate-200 bg-white"}`}
                      >
                        {/* Variant Header */}
                          ...
                          {variant.media.length > 0 && (
                            <div className="flex gap-2 mb-2 flex-wrap">
                              {variant.media.map((m, mIdx) => (
                                <div
                                  key={mIdx}
                                  className={`relative group w-16 h-16 rounded-lg overflow-hidden border-2 ${m.is_primary ? "border-blue-500" : "border-slate-200"}`}
                                >
                                  <img
                                    src={m.url}
                                    alt={`Variant ${vIdx} img ${mIdx}`}
                                    className="w-full h-full object-cover"
                                  />
                                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleRemoveVariantMedia(vIdx, mIdx)
                                      }
                                  ...
                                  {m.is_primary && (
                                    <span className="absolute top-0.5 left-0.5 text-[7px] font-bold bg-blue-500 text-white px-1 py-0.5 rounded">
                                      ★
                                    </span>
                                  )}
                                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditData((d) => ({ ...d, badge: e.target.value }))
                    }
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500"
                  >
                    {BADGES.map((b) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="text-[10px] text-slate-400 mt-1">
                    Harga yang dibayar customer di web public
                  </p>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">
                  ...
                  />
                  <p className="text-[10px] text-slate-400 mt-1">
                    Ditampilkan sebagai harga coret jika lebih tinggi dari Harga
                    Jual
                  </p>
                </div>
              </div>

              {/* B2B Price + Discount + Stock */}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setEditData((d) => ({
                        ...d,
                        stock: val,
                        inStock: val === "" ? false : val > 0,
                      }));
                    }}
                    className={`w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 disabled:bg-slate-100 disabled:text-slate-400 ${variants.length > 0 ? "cursor-not-allowed" : "cursor-text"}`}
                  />
                  {variants.length > 0 ? (
                    <p className="text-[9px] text-violet-600 font-semibold mt-1">
                      Dihitung otomatis dari total varian
                    </p>
                  ) : (
                    <p className="text-[10px] text-slate-400 mt-1">
                      Jumlah stok utama
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-slate-400 mt-1">
                  Copy-paste link YouTube/TikTok untuk ditampilkan di halaman
                  detail produk
                </p>
              </div>
              ...
              <div className="flex gap-6">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setEditData((d) => ({ ...d, inStock: e.target.checked }))
                    }
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setEditData((d) => ({ ...d, isNew: e.target.checked }))
                    }
                    className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-sm text-slate-700 font-medium">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md border border-slate-200/60 p-6 text-center">
            ...
            <p className="text-sm text-slate-500 mb-6">
              ...
              akan dihapus dari website public. Tindakan ini tidak dapat
              dibatalkan.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteTarget(null)}
                className="flex-1 px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all"
              >
                Batal
              </button>
              <button
                onClick={confirmDelete}
                className="flex-1 px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-bold transition-all"
              >
                Ya, Hapus
              </button>
            </div>
