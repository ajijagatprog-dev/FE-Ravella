...
export default function ReviewManagementPage() {
    ...
    const handleReply = async (id: number) => {
        try {
            setActionLoading(id);
            const res = await api.put(`/admin/reviews/${id}/reply`, { admin_reply: replyText });
            if (res.data.status === 'success') {
    ...
    const StatusBadge = ({ status }: { status: string }) => {
        ...
        const currentStyle = styles[lowerStatus] || "bg-stone-50 text-stone-500 border-stone-200";

        return (
            <span className={cn("text-[10px] font-bold px-2.5 py-1 rounded-full border uppercase tracking-wider", currentStyle)}>
                {status}
            </span>
        );
    };
        ...
        <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-8">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-32 space-y-4">
                    ...
                    <p className="text-sm font-medium text-gray-500">Memuat data ulasan...</p>
                </div>
            ) : reviews.length === 0 ? (
                <div className="bg-white border border-gray-100 rounded-3xl p-20 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm text-gray-400 mt-2 max-w-xs">Ulasan yang Anda cari tidak ditemukan atau belum ada data masuk.</p>
                </div>
            ) : (
                <div className="grid gap-6">
                    {reviews.map((review) => (
                        <div key={review.id} className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow">
                            <div className="flex flex-col lg:flex-row gap-8">
                                ...
                                <div className="lg:w-1/3 flex items-start gap-4 pr-6 lg:border-r border-gray-50">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-sm font-bold text-gray-900 truncate">{review.product.name}</p>
                                        <div className="flex items-center gap-2 mt-1">
                                            <div className="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center text-blue-600">
                                                <User size={12} />
                                            </div>
                                            <p className="text-xs font-medium text-gray-500 truncate">{review.user.name}</p>
                                        </div>
                                        <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-tighter">ORDER #{review.order_id}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div className="flex gap-1">
                                            {[1,2,3,4,5].map(s => (
                                                <Star key={s} size={14} className={cn(s <= review.rating ? "fill-amber-400 text-amber-400" : "text-gray-200")} />
                                            ))}
                                        </div>
                                        <span className="text-[11px] font-medium text-gray-400">{new Date(review.created_at).toLocaleString()}</span>
                                    </div>
                                    ...
                                    {review.admin_reply ? (
                                        <div className="ml-6 flex items-start gap-3 bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-bold text-blue-900 uppercase tracking-widest mb-1">Balasan Anda</p>
                                                <p className="text-sm text-blue-700 italic">{review.admin_reply}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="ml-6 space-y-3 animate-in slide-in-from-top-2">
                                            ...
                                            <div className="flex justify-end gap-2">
                                                ...
                                                >
                                                    {actionLoading === review.id ? <Loader2 size={12} className="animate-spin" /> : null}
                                                    Kirim Balasan
                                                </button>
                                            </div>
                                        </div>
                                ...
                                <div className="lg:w-48 flex flex-col gap-2 justify-center pl-6 lg:border-l border-gray-50">
                                    ...
                                    {review.status === 'approved' && !review.admin_reply && !replyingTo && (
                                        <button 
